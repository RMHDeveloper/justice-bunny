...
import { MessageDisplay } from './MessageDisplay'; // Keep for general error messages
import { StructuredLegalResult } from '../types';

interface LegalResultsProps {
  result: StructuredLegalResult | string | null; // Updated type
  loading: boolean;
  error: string | null;
  onStartNew: () => void;
}
...
export const LegalResults: React.FC<LegalResultsProps> = ({ result, loading, error, onStartNew }) => {
  const [isGuidanceExpanded, setIsGuidanceExpanded] = useState(false); // State for accordion
  ...
  const renderContent = () => {
    if (loading) {
      return (
        <div className="flex flex-col items-center justify-center flex-1 text-[#1A2B48]">
          <svg
            className="animate-spin -ml-1 mr-3 h-8 w-8 text-[#1A2B48]"
    ...
    return (
      <>
        ...
        {/* Display Explanation (Legal Guidance) as an Accordion */}
          ...
          <div>
            ...
            <ol className="list-decimal list-inside space-y-2 text-base md:text-lg">
              {structuredResult.procedure.map((step, index) => (
                <li key={index} className="pl-2 flex items-start"> {/* Added flex and items-start */}
                  <span className="font-bold text-lg md:text-xl text-[#1A2B48] mr-2"> {/* Bold number, dark blue */}
                    {index + 1}. {/* Always display the number */}
                  </span>
                  <span className="font-normal text-lg md:text-xl text-[#1A2B48] flex-1"> {/* Normal weight content, dark blue, fills remaining space */}
                    {/* Remove numbering if AI already provides it, otherwise display content */}
  ...
  return (
    <div className="flex flex-col p-6 bg-[#F8FAFC]">
      ...
      {/* <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 text-[#1A2B48] text-sm text-center mb-6">
        Disclaimer: This information is AI-generated and for informational purposes only. It does not constitute legal advice and should not be used as a substitute for consulting with a qualified legal professional.
      </div> */}
